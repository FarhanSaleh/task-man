@props(['label'])
<div>
    <button class="font-bold btn btn-primary" onclick="my_modal_0.showModal()">
        + Tambah Tugas
    </button>
    <dialog id="my_modal_0" class="modal">
        <div class="max-w-2xl modal-box">
            ...
            <h3 class="text-lg font-bold">Tambah Tugas</h3>
            ...
            <form action="{{ route('task.store') }}" method="post">
                ...
                <div class="flex gap-4">
                    <label class="w-full form-control">
                        ...
                        <input type="date" name="deadline" class="w-full input input-bordered"
                            value="{{ old('deadline') }}" />
                        @error('deadline')
                        <div class="text-red-600">{{ $message }}</div>
                        @enderror
                ...
                <label class="form-control">
                    ...
                    <textarea class="h-24 textarea textarea-bordered" name="description"
                        placeholder="Isi deskripsi tugas anda">{{ old("description") }}</textarea>
                </label>
